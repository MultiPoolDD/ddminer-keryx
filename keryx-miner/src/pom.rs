...
/// Per-device resident possession index + tier id. A mixed-GPU rig can mine a DIFFERENT model on
/// each GPU (the model that fits its VRAM), so the index — which is model-specific — is keyed by
/// CUDA device ordinal. Stored behind `Arc` so a caller can hold it across the (slow) host re-walk
/// without keeping the registry lock. The legacy single-GPU case is just device 0.
/// Per-device index keyed alongside the model_id it was built from. The PoM TIER is NOT stored
/// here: post-H2 the tier index is daa-dependent, so it must be recomputed per block (see
/// `pom_gpu::current_tier`) rather than frozen at index-build time.
static POM_INDICES: Mutex<BTreeMap<u32, (Arc<WeightIndex>, [u8; 32])>> = Mutex::new(BTreeMap::new());

/// Install device `dev`'s possession index, tagged with the `model_id` it was built from.
pub fn set_index(dev: u32, model_id: [u8; 32], index: WeightIndex) {
    POM_INDICES
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .insert(dev, (Arc::new(index), model_id));
}
...
pub fn try_share_index(dev: u32, model_id: &[u8; 32]) -> bool {
    let mut g = POM_INDICES.lock().unwrap_or_else(|e| e.into_inner());
    if g.contains_key(&dev) {
        return true;
    }
    let sibling = g.values().find(|(_, m)| m == model_id).map(|(a, _)| a.clone());
    if let Some(arc) = sibling {
        g.insert(dev, (arc, *model_id));
        true
    } else {
        false
    }
...
mod tests {
    ...
    fn h3_salt_changes_seed_and_pow_and_roundtrips() {
        ...
        let proof = build_proof(1, &pph, nonce, seed_h3, idx.n_chunks, k, t, |o| idx.read_chunk(o), |o| idx.merkle_path(o), true);
