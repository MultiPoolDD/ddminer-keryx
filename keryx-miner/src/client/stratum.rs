...
/// (accepted, rejected) para la línea de estado del miner. rejected = stale+low_diff+duplicate.
pub fn share_counts() -> (u64, u64) {
    match SHARE_STATS.get() {
        Some(s) => (
            s.accepted.load(Ordering::SeqCst),
            s.stale.load(Ordering::SeqCst) + s.low_diff.load(Ordering::SeqCst) + s.duplicate.load(Ordering::SeqCst),
        ),
        None => (0, 0),
    }
}

/// Nombre del rig para el worker en stratum: hostname del equipo, saneado (primer
/// label, solo alfanumérico + - _, máx 32). Si no hay hostname → "rig".
fn rig_worker_name() -> String {
...
impl Client for StratumHandler {
    ...
    async fn register(&mut self) -> Result<(), Error> {
            ...
            .send(StratumLine {
                ...
                payload: StratumLinePayload::StratumCommand(StratumCommand::Subscribe(
                    MiningSubscribe::MiningSubscribeOptions((
                        format!("ddminer/{}", env!("CARGO_PKG_VERSION")),
        ...
        self.send_channel
            .send(StratumLine {
                ...
                payload: StratumLinePayload::StratumCommand(StratumCommand::Authorize((
                    // worker = direccion.HOSTNAME para que el pool identifique el rig automáticamente.
                    // La dirección interna (miner_address) no se toca; solo el usuario del authorize.
                    format!("{}.{}", pay_address, rig_worker_name()),
                    "x".into(),
                ))),
                jsonrpc: None,
                error: None,
        ...
        // Declare loaded SLM models so the bridge challenges with the right model.
        self.declare_capabilities().await?;
        Ok(())
    }
    ...
    async fn listen(&mut self, miner: &mut MinerManager) -> Result<(), Error> {
        ...
        loop {
            {
                // El time-slice de identidad user<->devfund solo aplica si el devfund
                // esta habilitado. Con devfund off (address None / percent 0) NUNCA debe
                // hacer este early-return: si no, con devfund_percent=0 la condicion
                // `ctr <= 0` se cumple cuando block_template_ctr cicla a 0 y entra en un
                // reconnect-storm permanente (cierra antes de procesar ningun notify).
            ...
            // Watch the pool read-stream AND the submit task together. The submit task ends when the
            // pool's WRITE half closes (send_channel error → break); if we only awaited the read
            // stream we'd keep running while every found share floods a dead submit channel with
            // "channel closed". Ending listen() here makes client_main return → the outer loop
            // reconnects and rebuilds a live submit channel.
            let msg = {
                let stream = &mut self.stream;
                let block_handle = &mut self.block_handle;
                tokio::select! {
                    m = stream.try_next() => m?,
                    _ = block_handle => return Err("submit task ended — reconnecting".into()),
                }
            };
            match msg {
                Some(msg) => self.handle_message(msg, miner).await?,
                // Framed stream Ok(None) = the pool closed the socket (peer FIN), NOT an empty
                // payload. This is almost always a deliberate server-side disconnect — most often an
                // OPoI kick because the declared model_id is not in the on-chain registry. Return Err
                // (not Ok) so the outer loop's backoff throttles the reconnect instead of hammering
                // the pool. The message is actionable so users check their config before reporting.
                None => {
                    return Err("connection closed by pool — usually an OPoI kick: the declared \
                                model must be a valid PoM tier (gemma-3-4b, dolphin-8b, qwen3-32b, \
                                llama-3.3-70b; qwen3-1.7b/70b-q2 post-H2). Update to the latest \
                                release or check your --light/--high/--very-high tier."
                        .into());
                }
            }
        }
...
impl StratumHandler {
    /// Send `mining.declare_capabilities` with the currently-served lineup. Called at connect and
    /// again right after the OPoI v2 swap, so post-hardfork the bridge challenges (and rewards) the
    /// PoM mining model — not the stale legacy lineup we declared before the first job arrived.
    async fn declare_capabilities(&self) -> Result<(), Error> {
    ...
    fn create_block_channel(
        ...
        let handle = tokio::spawn(async move {
            ...
            while let Some(seed) = recv_stream.next().await {
                ...
                let line = if let Some(proof_bytes) = pom_proof {
                    // PoM possession submit (post-activation): the mining loop only attaches a
                    // proof when the held job's daa_score >= POM_ACTIVATION_DAA. Send ONE 6-tuple
                    // mining.submit carrying the opoi_tag, the ipfs_cid ("" if no inference) and
                    // the hex-encoded possession proof, so the pool reinjects it into
                    // Block.pom_proof before submitBlock (a PoM block is invalid without it).
                    let proof_hex = hex::encode(&proof_bytes);
                    let cid = cid_opt.unwrap_or_default();
                    info!(
                        "PoM: submitting 6-tuple share with possession proof ({} B → {} hex chars) for job {}",
                        proof_bytes.len(),
                        proof_hex.len(),
                        job_id
                    );
                    StratumLine {
                        ...
                        payload: StratumLinePayload::StratumCommand(StratumCommand::MiningSubmit(
                            MiningSubmit::MiningSubmitWithPomProof((
                                miner_address.clone(),
                                job_id,
                                nonce_hex,
                                opoi_tag,
            ...
            StratumLine { id, payload, error: None, .. } => {
                match payload {
                        ...
                        match result {
                            StratumResult::Plain(Some(true)) | StratumResult::Eth((true, _)) => {
                                    ...
                                    .remove(&id.expect("We checked id is not none"))
                                {
                                    ...
                                    log::debug!("Ignoring result for now");
                                }
                                Ok(())
                            }
                    ...
                    StratumLinePayload::StratumCommand(command) => match command {
                        ...
                        // EthereumStratum/1.0.0 (NiceHash-style) set_extranonce: solo el extranonce
                        // hex, sin nonce_size. El ancho lo da la longitud (2 bytes -> "0072"); el
                        // minero controla los bytes bajos restantes (8 - ancho_extranonce).
                        StratumCommand::SetExtranonce(SetExtranonce::SetExtranoncePlainEth((
                            ref extranonce,
                        ))) => {
                            let nonce_size = 8u32.saturating_sub((extranonce.len() as u32) / 2);
                            self.set_extranonce(extranonce.as_str(), &nonce_size)
                        }
                        ...
                        StratumCommand::MiningNotify(MiningNotify::MiningNotifyWithTask((
                            ...
                            self.block_template_ctr
                                ...
                                .unwrap();
                            // OPoI v2 hardfork: hot-swap the served/mining lineup once the chain
                            // reaches OPOI_V2_ACTIVATION_DAA (mirrors solo grpc.rs). Without this the
                            // pool path never registers the PoM mining model in SUPPORTED_SPECS, so
                            // ensure_loaded() can't find it and the GPU PoM miner stalls in "warming up".
                            if keryx_miner::slm::advance_lineup_if_due(daa_score) {
                                // Lineup just swapped to the uncensored/PoM model — re-declare so the
                                // bridge challenges (and rewards) the PoM model, not the legacy lineup
                                // we declared at connect. Otherwise it may keep challenging e.g.
                                // deepseek-r1-8b, thrashing the GPU between that and the Dolphin PoM
                                // walk and stalling PoW in "warming up".
                                self.declare_capabilities().await?;
                            }
            ...
            StratumLine {
                ...
                match code {
                    ErrorCode::Unknown => {
                        ...
                        log::debug!("Share rejected by pool (Job id: {:?}): {}", jobid, error);
                        Ok(())
                    }
                    ErrorCode::JobNotFound => {
                        self.shares_stats.stale.fetch_add(1, Ordering::SeqCst);
                        log::debug!("Stale share (Job id: {:?})", jobid);
                        Ok(())
                    }
                    ErrorCode::DuplicateShare => {
                        self.shares_stats.duplicate.fetch_add(1, Ordering::SeqCst);
                        log::debug!("Duplicate share (Job id: {:?})", jobid);
                        Ok(())
                    }
                    ErrorCode::LowDifficultyShare => {
                        self.shares_stats.low_diff.fetch_add(1, Ordering::SeqCst);
                        log::debug!("Low difficulty share (Job id: {:?})", jobid);
                        Ok(())
                    }
    ...
    async fn log_shares(_shares_info: Arc<ShareStats>) {
        // Las shares se muestran en la línea de estado única del miner (miner.rs::log_hashrate).
        // Esta tarea queda inactiva para no imprimir una segunda línea que rompa el \r.
    ...
    async fn handle_challenge(&mut self, model_id_hex: String, nonce_hex: String, miner: &mut MinerManager) {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if text.is_empty() {
                log::debug!("OPoI challenge: inference returned empty text for model {:.8}", model_id_hex);
            } else {
