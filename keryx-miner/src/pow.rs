...
pub enum BlockSeed {
    ...
    PartialBlock {
        ...
        nonce_fixed: u64,
        hash: Option<String>,
        /// PoM possession proof (borsh-encoded `pom::PomProof`) for POOL submission via
        /// `mining.submit_pom`. `None` on the legacy kHeavyHash path. When set, the miner
        /// found a PoM nonce against the (share or block) target and the pool reinjects
        /// these bytes into `Block.pom_proof` before `submitBlock`.
...
impl State {
    ...
    pub fn generate_block_if_pom(&self, nonce: u64, index: &WeightIndex, tier: u8) -> Option<BlockSeed> {
        ...
        match block_seed {
            BlockSeed::FullBlock(ref mut block) => {
                // Solo path: the proof rides the submitted block directly.
            ...
            // Pool path: a PartialBlock cannot carry the proof in the block body (the pool
            // builds the block), so stash it on the seed. stratum.rs sends it as the 6th
            // param (pom_proof_hex) of mining.submit and the pool reinjects it into
            // Block.pom_proof.
            BlockSeed::PartialBlock { nonce: ref mut header_nonce, pom_proof: ref mut pp, .. } => {
                *header_nonce = nonce;
                *pp = Some(bytes);
            }
        }
